...
module.exports = {
  ...
  //返回对应码的名称
  getNameByCode: function (list, code) {
    var arr = list.filter(function (item) {
      return item.value === code && item;
    });
  ...
  getCityList: function (code) {
    ...
    return this.getItems(this.city(), province);
  },

  //获取县列表
  getDistrictList: function (code) {
    code = this.convertCode(code);
    if (code.length < 4) {
      return;
    }
    ...
    return this.getItems(this.district(), cityCode);
  },

  //根据省编码获取省的名称
  getProvNameByCode: function (code) {
    code = this.convertCode(code);
    if (code.length < 2) {
      return;
    }
    var province = code.slice(0, 2) + '0000';
    return this.getNameByCode(this.getProvinceList(), province);
  },
  ...
  getCityNameByCode: function (code) {
    ...
    var cityCode = code.slice(0, 4) + '00';
    return this.getNameByCode(this.getCityList(code), cityCode);

  },

  //根据地区编码获取地区的名称
  getDistrictNameByCode: function (code) {
    code = this.convertCode(code);
    if (code.length < 6) {
      return;
    }
  ...
  getAreaByCode: function (code) {
    ...
    return this.getProvNameByCode(code) + this.getCityNameByCode(code) + this.getDistrictNameByCode(code);
  }
